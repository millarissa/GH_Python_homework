...
class CurrentMoneyCourse:
    SITE_API_URL = 'https://api.privatbank.ua/p24api/pubinfo?json&exchange&coursid=5'

    def get_money_course(self):
        api_courses = requests.get(self.SITE_API_URL).json()
        for course in range(len(api_courses)):
            api_courses[course].pop('base_ccy')

        headers = {'ccy': 'Currency', 'buy': 'Buy', 'sale': 'Sale'}
        print('Current money course to UAH:')

        print(tabulate(api_courses, headers=headers, tablefmt='grid'))
        return api_courses
